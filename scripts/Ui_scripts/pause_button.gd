...
func _ready():
	...
	# Set all AudioStreamPlayer Pause Mode to PROCESS so sounds work while paused
	if pause_click_sound:
		pause_click_sound.process_mode = Node.PROCESS_MODE_ALWAYS
	if resume_click_sound:
		resume_click_sound.process_mode = Node.PROCESS_MODE_ALWAYS
	if main_menu_click_sound:
		main_menu_click_sound.process_mode = Node.PROCESS_MODE_ALWAYS
...
func _on_pause_pressed():
	...
	if pause_canvas.visible:
		# If pause menu is visible, close it and unpause
		pause_canvas.visible = false
		get_tree().paused = false
	else:
		# If pause menu is hidden, open it and pause the game

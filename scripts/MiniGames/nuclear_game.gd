extends Node2D  # Attached to DumpyardController

@onready var area = $Area2D
@onready var interact_label = $InteractLabel
@onready var tilemap_layer = $PowerPlant  # TileMapLayer node
...
func _input(event):
	if can_interact and event.is_action_pressed("interact"):
		...
		get_tree().change_scene_to_file("res://scenes/Mini_games_level_Screens/NuclearLevels.tscn")

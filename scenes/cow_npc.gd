...
var waypoints = [
	...
	Vector2(846.0, 843.0),   # Point 3 - grass area
...
func _physics_process(delta):
	match current_state:
		CowState.WALKING:
			walk_to_current_waypoint(delta)
		CowState.EATING:
...
func walk_to_current_waypoint(delta):
	if waypoints.size() == 0:
		return
...
func move_to_next_waypoint():
	...
	print("Moving to waypoint ", current_waypoint_index, ": ", waypoints[current_waypoint_index])
...
func change_state(new_state: CowState):
	...
	match current_state:
		CowState.WALKING:
			play_current_animation()
			print("Cow walking to waypoint ", current_waypoint_index)
		CowState.EATING:
			...
			timer.start()
			print("Cow eating for ", eating_time, " seconds")
		CowState.IDLE:
